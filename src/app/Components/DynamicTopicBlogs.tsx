...
interface DynamicTopicBlogs {
  initialBlogs: BlogPreview[];
  username: string;
  topic: string;
}
export default function DynamicTopicBlogs({
  initialBlogs,
  username,
  topic,
}: DynamicTopicBlogs) {
  ...
  // State to track which reaction is selected per blog (blogId => reaction type)
  ...
  const loadMoreBlogs = useCallback(async () => {
    setLoadingMore(true);
    try {
      const data = await getBlogsByTopic(
        username,
        topic,
        offsetRef.current,
        10,
