...
interface ProfileHighlightedBlogsProps {
...
export default function MyProfileHighlightedBlogs({
  ...
  const handleHighlightBlog = async (slug: string) => {
    try {
      ...
      setTimeout(async () => {
        const data = await getHighlightedBlogs(0, 10, username);
        if (data.blogs) {
          setBlogs(data.blogs);
